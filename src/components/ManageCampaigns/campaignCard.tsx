...
import { Row, Col, Form, Overlay, Spinner } from "react-bootstrap";
...
import InviteUsers from "../../components/InviteInvestors/inviteUsers";
...
//save campaign image to s3 bucket
  ...
  const upload = async (e) => {
    ...
    setFileName(state.inputs.title.replace(/\s/g, '') + "_" + e.target.files[0].name.replace(/\s/g, ''));
                      ...
                      <Form.Group>
                        <label className="w-100">
                          <div className='position-relative'>
                            <Form.Control
                              type="text"
                              placeholder={state.isUpdateMode ? `${strings.campaignUrl}` : ""}
                              name="campaignUrl"
                              onChange={(e) => {
                                handleChange(e);
                              }}
                              value={state.isUpdateMode ? state.inputs.campaignUrl ? state.inputs.campaignUrl : '' : ''}
                              disabled={!state.isUpdateMode}
                            />
                            <a href={state.inputs.campaignUrl} className={`position-absolute dln-donationURL text-truncate ${state.isUpdateMode && 'd-none'}`} target="_blank" rel="noopener noreferrer">
                      ...
                      <Form.Group>
                        <label className="w-100">
                          <div className='position-relative'>
                            ...
                            <Form.Control
                              type="text"
                              placeholder={state.isUpdateMode ? "Donation URL" : ""}
                              name="donationUrl"
                              onChange={(e) => {
                                handleChange(e);
                              }}
                              value={state.isUpdateMode ? state.inputs.donationUrl ? state.inputs.donationUrl : '' : ''}
                              disabled={!state.isUpdateMode}
                            />
                            <a href={state.inputs.donationUrl} className={`position-absolute dln-donationURL text-truncate ${state.isUpdateMode && 'd-none'}`} target="_blank" rel="noopener noreferrer">
