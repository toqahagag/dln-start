...
function Home() {
  ...
  const getProposals = async () => {
    ...
    if (response) {
      if (response.success) {
        setLoanData({totalFunded:response.data.funded.amount,totalRepaid:response.data.paid})
      } else {
      }
    }
        ...
        <Col xl={7} lg={7} md={7} sm={12}>
          ...
          >
            <Tab eventKey="borrow" title={borrowText}>
             {selectedKey==="borrow" &&  <Borrow handleCallSpinner={handleCallSpinner} borrowProposals={proposals.filter((item) => item.status !== 6)} loanData={loanData} />}
            </Tab>

            <Tab eventKey="repaid" title={repaidText}>
             {selectedKey==="repaid" &&  <Repaid  handleCallSpinner={handleCallSpinner} repaidProposals={proposals.filter((item) => item.status === 6)} loanData={loanData}/>}
            </Tab>
