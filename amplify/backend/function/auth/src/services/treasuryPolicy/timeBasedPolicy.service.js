...
const _DepositWalletContract = (depositAddress) => new web3.eth.Contract(ERC20Wallet, depositAddress).methods;
const GenericERC20Contract = (tokenAddress) => new web3.eth.Contract(GenericERC20, tokenAddress).methods;
...
const addDMMTokenToTreasury = async () => {
    for (const token of tokens.DMMAssets) {
        ...
        const addDMMWalletTokenToTreasuryWallet = await _DepositWalletContract(TestDMMWalletAddr)
...
const addTreasuryTokenToDMM = async () => {
    for (const token of tokens.testAssets) {
        ...
        const addTreasuryWalletTokenToDMM = await _DepositWalletContract(TreasuryWalletAddr)
